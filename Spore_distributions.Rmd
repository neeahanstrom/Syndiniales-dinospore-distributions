...
```{r}
keep <- ps_spores %>%                                                        
  subset_taxa(Division == "Alveolata") # <- removes the crustacean reads
...
# Check the read counts in the samples
sample_reads <- sample_sums(filtered_relative) # The total alveolata reads in all samples after the filtrations
...
ggrare(physeq_object = filtered_relative,
       step = 100,
       parallel = TRUE) +
  geom_line(color = "navy") +   # Add the colour you want
...
```
# Get the reads of Alveolata in the water and in the host samples

```{r}
physeq_subset <- subset_taxa(filtered_relative, Division == "Alveolata") ### You can change the taxonomic level and the name of the taxa of interest accordingly here
...
reads_by_sample_type <- sample_data_df %>%
  group_by(Sample_type) %>% #Here you can change the gouping (for example to Filter_size)
  summarise(Total_Reads = sum(Total_Reads))

#Print the total reads of Alveolata in water and in the hosts
print(reads_by_sample_type)

##############  same for the reads at other taxonomic levels: #############

physeq_subset <- subset_taxa(filtered_relative, Subdivision == "Dinoflagellata") ### You can change the taxonomic level and the name of the taxa of interest accordingly here
...
dfwater <- df %>%
  filter(Sample_type == "Water") # <- Only water/Zooplankton samples
...
# Group other clases as "Other" 
Alveolata <- Alveolata %>%
  mutate(Class_grouped = ifelse(Class %in% top_classes, Class, "Other")) #if the class is not in the top10, it will be in the group "other"
...
```
# Fig. S3 ASV richness and statistical analysis
```{r}
...
# all together
asv_richness <- df2 %>%
  filter(Taxa == "Syndiniales", Abundance > 0,
         Sorted_genus == "Water") %>%
  group_by(Station_ID, Sample) %>%
  summarise(nasv = n_distinct(OTU), .groups = "drop")

# set the plotting order of the locations
asv_richness$Station_ID <- factor(
  asv_richness$Station_ID,
  levels = c("A17", "BY5", "BY15", "BY31") #plot order of the locations
)


all <- ggplot(asv_richness, aes(x = Station_ID, y = nasv, color = factor(Station_ID))) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.6) +
  theme_minimal(base_size = 16) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = " ", y = "ASV richness, all water samples", color = "Station_ID") +
...
boxplot(nasv ~ Station_ID, data = asv_richness) # --> skewed distribution, do kruskall wallis

kruskal.test(nasv ~ Station_ID, data = asv_richness) #--> significant, do post hoc

# Dunn's test for post hoc
library(FSA)
dunnTest(nasv ~ Station_ID, data = asv_richness, method = "bh")
...
# set the plotting order of the locations
asv_richness20$Station_ID <- factor(
  asv_richness20$Station_ID,
  levels = c("A17", "BY5", "BY15", "BY31") #plot order of the locations
)


micro <- ggplot(asv_richness20, aes(x = Station_ID, y = nasv, color = factor(Station_ID))) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.6) +
  theme_minimal(base_size = 16) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = " ", y = "ASV richness, microplankton", color = "Station_ID") +
  scale_color_manual(values = c("slateblue4",
                                  "hotpink",
                                  "seagreen4",
                                  "lightblue")) +
...
# Check the distribution
boxplot(nasv ~ Station_ID, data = asv_richness20) # --> skewed distribution, do kruskall wallis

kruskal.test(nasv ~ Station_ID, data = asv_richness20) #--> significant, do post hoc

# Dunn's test for post hoc
dunnTest(nasv ~ Station_ID, data = asv_richness20, method = "bh")
...
# set the plotting order of the locations
asv_richness02$Station_ID <- factor(
  asv_richness02$Station_ID,
  levels = c("A17", "BY5", "BY15", "BY31") #plot order of the locations
)


pico <- ggplot(asv_richness02, aes(x = Station_ID, y = nasv, color = factor(Station_ID))) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.6) +
  theme_minimal(base_size = 16) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = " ", y = "ASV richness, pico-nanoplankton", color = "Station_ID") +
  scale_color_manual(values = c("slateblue4",
                                  "hotpink",
                                  "seagreen4",
                                  "lightblue")) +
...
boxplot(nasv ~ Station_ID, data = asv_richness02) # --> skewed distribution, do kruskall wallis

kruskal.test(nasv ~ Station_ID, data = asv_richness02) #--> if significant, do post hoc
...
```
# Fig. S4. SGs in the different size fractions
...
ggplot(pcoa_scores, aes(x = V1, y = V2)) +
  ...
  scale_color_manual(values=c("lightblue","hotpink", "seagreen4", "slateblue4")) +
...
# Permanova for the bray

P <- adonis2(diss_matrix ~ Station_ID + sample._strata + Filter_size,
             data = PCOA_wide_filtered,
             permutations = 999,
             by = "margin")
...
PCOA_mat <- PCOA_wide[,12:39] %>% as.matrix() # 39 for microplankton, 53 for pico-nano
...
# Plot the PCoA results
ggplot(pcoa_scores, aes(x = V1, y = V2)) +
  geom_point(mapping = aes(shape = sample_strata, col = Location), size=3) +
  scale_color_manual(values=c("lightblue","hotpink", "seagreen4", "slateblue4")) + #Choose the colour
  #geom_text(vjust = 1.5, hjust = 1.5) +
  labs(title = "Principal Coordinate Analysis (PCoA)", x = "PCoA 1", y = "PCoA 2") +
  #geom_label(data = df_species,
  #                    mapping = aes(x= NMDS1, y =NMDS2, label= species)) #+
...
env_scores <- as.data.frame(scores(env_fit, display = "vectors"))
env_scores$variable <- rownames(env_scores)  # Add variable names
env_scores$pval <- env_fit$vectors$pvals # Add p-values

# Keep only significant vectors (p < 0.05)
...
# Plot the PCoA results 
ggplot(pcoa_scores, aes(x = V1, y = V2)) + 
  geom_point(mapping = aes(shape = sample_strata, col = Location), size=3) + scale_color_manual(values=c("lightblue","hotpink", "seagreen4", "slateblue4")) + #Choose the colour 
  geom_segment(data = env_scores_sig, aes(x = 0, y = 0, xend = Dim1, yend = Dim2), 
               arrow = arrow(length = unit(0.3, "cm")), color = "black") + 
  geom_text(data = env_scores, 
            aes(x = Dim1, y = Dim2, label = " "), 
            vjust = 1.5, hjust = 1.5, color = "black") + 
  labs( x = paste0("PCoA1 (", round(variation_explained[1], 2), "%)"), 
        y = paste0("PCoA2 (", round(variation_explained[2], 2), "%)"), 
        title = "PCoA Plot with Variation Explained" ) + #If I want to have the dino groups showing in the plot 
  coord_fixed() +
  theme_classic()
...
# Plot the sampling station in the Baltic Sea -------------------------------- #
  ...
  ggplot() +
    # Baltic Sea silhouette
    geom_sf(data = sf,
            fill = "transparent",
            color = "black") +
    # Labels directly on the map
    geom_sf_text(data = coord_sf,
                 aes(label = Basin),
                 size = 4) +
    theme_void() +
...
# Fig. S1. Vertical prophiles
- plot the abiotic parameters into vetrical prophiles. Use the actual values, not averages calculated above (df = "abiotic")
- Change the parameter on the x-axis to plot different variables

```{r}
   # Plot the parameters in vertical prophile plots
  ggplot(abiotic %>% arrange(Station_ID, Depth),
         aes(x = Chlorophyll.a, y = Depth, color = Station_ID)) + #Change the parameter you want to plot (Salinity, Oxygen, Temperature ect...)
    geom_path() +
    geom_point(size = 2) +
    scale_y_reverse(limits = c(90, 0), breaks = seq(0, 90, by = 10)) +
    labs(
      y = "Depth (m)",
      x = "Chlorophyll-a μg/L", # Change the x-axis label accordingly to the parameters
      color = "Location"
    ) +
...
```
# Fig. 4. Shannon/Simpson
- Shannon diversity index
- Simpson index
- Analysis done on ASV level
```{r}
syndishannon <- syndiniales %>%
  select(Sample, Abundance, Filter_size, sample._strata, Station_ID, OTU, Sorted_genus) # Change to order of family level depending on what you are interested in

syndiwater <- syndishannon %>%
  filter(Sorted_genus == "Water")

syndi_long <-  syndiwater %>%
  group_by(Sample, OTU, sample._strata, Station_ID, Filter_size) %>%
  summarise(Abundance = mean(Abundance, na.rm = TRUE)) %>% # or median, or whatever you want
  pivot_wider(names_from = OTU, values_from = Abundance)

syndi_long$sample._strata <- factor(
  syndi_long$sample._strata,
  levels = c("Surface", "Medium", "Deep") # order of the strata
)

syndi_long$Station_ID <- factor(
  syndi_long$Station_ID,
  levels = c("A17", "BY5", "BY15", "BY31") #plot order of the locations
)

# Calculate Shannon diversity (H') 

syndi_long$Shannon <- diversity(syndi_long[, 41:ncol(syndi_long)], index = "shannon")


head(syndi_long)

shannon_summary_syndiniales <- syndi_long %>%
  group_by(Station_ID, sample._strata, Filter_size) %>%
  summarize(Mean_Shannon = mean(Shannon), .groups = "drop")

print(shannon_summary_syndiniales)

shannon20S <- syndi_long %>%
  filter(Filter_size == "20")

shannon02S <- syndi_long %>%
  filter(Filter_size == "0.2")

sha <- ggplot(syndi_long,
  aes(x = sample._strata, y = Shannon, color = factor(Filter_size))) +
  geom_point(size = 4) +
  facet_wrap(~Station_ID, nrow = 1) +
    theme_minimal(base_size = 16) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Water layer", y = "Shannon diversity", color = "Filter size (µm)") +
  scale_color_manual(values = c("navy", "pink")) +
  theme(
    strip.text = element_text(size = 14, face = "bold"),
    legend.position = "bottom"
  )
...
syndi_long$Simpson <- diversity(syndi_long[, 41:ncol(syndi_long)], index = "simpson")

simpson_summary_syndiniales <- syndi_long %>%
  group_by(Station_ID, sample._strata, Filter_size) %>%
  summarize(Mean_Simpson = mean(Simpson), .groups = "drop")

print(simpson_summary_syndiniales)

simpson20S <- syndi_long %>%
  filter(Filter_size == "20")

simpson02S <- syndi_long %>%
  filter(Filter_size == "0.2")

sim <- ggplot(syndi_long,
  aes(x = sample._strata, y = Simpson, color = factor(Filter_size))) +
  geom_point(size = 4) +
  facet_wrap(~Station_ID, nrow = 1) +
  theme_minimal(base_size = 16) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Water layer", y = "Simpson diversity", color = "Filter size (µm)") +
  scale_color_manual(values = c("navy", "pink")) +
  theme(
    strip.text = element_text(size = 14, face = "bold"),
    legend.position = "bottom"
