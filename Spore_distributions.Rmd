...
- Remove all but Alveolata reads from the phyloseq object and get the read counts of the taxa included in this study
- Plot the rarefaction curves

```{r}
keep <- ps_spores %>%                                                        
  subset_taxa(Division == "Alveolata") # <- Keeping only the Alveolata reads
...
ggplot(pcoa_scores, aes(x = V1, y = V2)) +
  ...
  geom_segment(data = env_scores, aes(x = 0, y = 0, xend = Dim1, yend = Dim2), 
               arrow = arrow(length = unit(0.3, "cm")), color = "black") +
  geom_text(data = env_scores, aes(x = Dim1, y = Dim2, label = " "), 
            vjust = 1.5, hjust = 1.5, color = "black") +
  labs(
    x = paste0("PCoA1 (", round(variation_explained[1], 2), "%)"),
    y = paste0("PCoA2 (", round(variation_explained[2], 2), "%)"),
...
ggplot() +
  ...
  scale_fill_manual(values = c("lightblue","hotpink", "seagreen4", "slateblue4")) +
